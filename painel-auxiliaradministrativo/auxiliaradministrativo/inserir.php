...
if($antigo != $cpf){
	$query = $pdo->query("SELECT * FROM auxiliaradministrativo where cpf = '$cpf' ");
...
if($antigo2 != $email){
	$query = $pdo->query("SELECT * FROM auxiliaradministrativo where email = '$email' ");
	$res = $query->fetchAll(PDO::FETCH_ASSOC);
	$total_reg = @count($res);
	if($total_reg > 0){
...
if($id == ""){
	$res = $pdo->prepare("INSERT INTO auxiliaradministrativo SET nome = :nome, cpf = :cpf, email = :email, telefone = :telefone, escola = :escola");
	...
	$res2->bindValue(":nivel", 'auxadministrativo');

}else{
	$res = $pdo->prepare("UPDATE auxiliaradministrativo SET nome = :nome, cpf = :cpf, email = :email, telefone = :telefone, escola = :escola WHERE id = '$id'");
