...
if($antigo != $cpf){
	$query = $pdo->query("SELECT * FROM coordenadiretor where cpf = '$cpf' ");
...
if($antigo2 != $email){
	$query = $pdo->query("SELECT * FROM coordenadiretor where email = '$email' ");
	$res = $query->fetchAll(PDO::FETCH_ASSOC);
	$total_reg = @count($res);
	if($total_reg > 0){
...
if($id == ""){
	$res = $pdo->prepare("INSERT INTO coordenadiretor SET nome = :nome, cpf = :cpf, email = :email, telefone = :telefone, escola = :escola");
	...
	$res2->bindValue(":nivel", 'coordenadiretor');

}else{
	$res = $pdo->prepare("UPDATE coordenadiretor SET nome = :nome, cpf = :cpf, email = :email, telefone = :telefone, escola = :escola WHERE id = '$id'");
