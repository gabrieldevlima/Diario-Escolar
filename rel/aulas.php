...
if(strpos($nome_escola, 'U. E. FRANCISCO JOSÉ DOS SANTOS') !== false){
	$html = file_get_contents($url."rel/aulas_francisco_html.php?id=$id");
	echo $html;
}

if(strpos($nome_escola, 'CRECHE MUNICIPAL PROFª MARIA MADALENA') !== false){
	$html = file_get_contents($url."rel/aulas_creche_html.php?id=$id");
	echo $html;
}

if(strpos($nome_escola, 'E. M. NÚCLEO DA ALEGRIA') !== false){
	$html = file_get_contents($url."rel/aulas_nucleo_html.php?id=$id");
	echo $html;
}

if(strpos($nome_escola, 'E. M. NOVO HORIZONTE') !== false){
	$html = file_get_contents($url."rel/aulas_horizonte_html.php?id=$id");
	echo $html;
}
if(strpos($nome_escola, 'E. M. SÃO JOSÉ') !== false){
	$html = file_get_contents($url."rel/aulas_saojose_html.php?id=$id");
	echo $html;
}
