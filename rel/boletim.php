...
$query_m = $pdo->query("SELECT * FROM matriculas where aluno = '$aluno'");
...
if (strpos($nome_escola, 'U. E. FRANCISCO JOSÉ DOS SANTOS') !== false) {
	$html = file_get_contents($url . "rel/boletim_francisco_html.php?id=$aluno");
	echo $html;
}

if (strpos($nome_escola, 'E. M. SÃO JOSÉ') !== false and $serie >= 3) {
	$html = file_get_contents($url . "rel/boletim_saojose_html.php?id=$aluno");
	echo $html;
} else if (strpos($nome_escola, 'E. M. SÃO JOSÉ') !== false and $nome_disc == 'E. INFANTIL') {
	$html = file_get_contents($url . "rel/boletim_infan_saojose_html.php?id=$aluno");
	echo $html;
} else if (strpos($nome_escola, 'E. M. SÃO JOSÉ') !== false and $serie < 3) {
	$html = file_get_contents($url . "rel/boletim_menor_saojose_html.php?id=$aluno");
	echo $html;
}

if (strpos($nome_escola, 'E. M. NOVO HORIZONTE') !== false and $serie >= 3) {
	$html = file_get_contents($url . "rel/boletim_novohorizonte_html.php?id=$aluno");
	echo $html;
}


if (strpos($nome_escola, 'E. M. NOVO HORIZONTE') !== false and $serie < 3) {
	$html = file_get_contents($url . "rel/boletim_novohorizonte_menor_html.php?id=$aluno");
	echo $html;
}

if (strpos($nome_escola, 'E. M. NÚCLEO DA ALEGRIA') !== false) {
	$html = file_get_contents($url . "rel/boletim_nucleo_html.php?id=$aluno");
	echo $html;
}

if (strpos($nome_escola, 'CRECHE MUNICIPAL PROFª MARIA MADALENA') !== false) {
	$html = file_get_contents($url . "rel/boletim_creche_html.php?id=$aluno");
	echo $html;
}