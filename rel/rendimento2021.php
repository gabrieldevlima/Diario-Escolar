...
if (strpos($nome_escola, 'E. M. SÃO JOSÉ') !== false and $serie >= 3) {
	$html = file_get_contents($url . "rel/rendimento_saojose_html2021.php?id=$id");
	echo $html;
} else if (strpos($nome_escola, 'E. M. SÃO JOSÉ') !== false and $nome_disc == 'E. INFANTIL') {
	$html = file_get_contents($url . "rel/rendimento_infan_saojose_html2021.php?id=$id");
	echo $html;
} else if (strpos($nome_escola, 'E. M. SÃO JOSÉ') !== false and $serie <= 2 and $serie > 0) {
	$html = file_get_contents($url . "rel/rendimento_menor_saojose_html2021.php?id=$id");
	echo $html;
}

if (strpos($nome_escola, 'E. M. NOVO HORIZONTE') !== false and $serie < 3) {
	$html = file_get_contents($url . "rel/rendimento_novohorizonte_menor_html2021.php?id=$id");
	echo $html;
}

if (strpos($nome_escola, 'E. M. NOVO HORIZONTE') !== false and $serie >= 3) {
	$html = file_get_contents($url . "rel/rendimento_novohorizonte_html2021.php?id=$id");
	echo $html;
}

if (strpos($nome_escola, 'E. M. NÚCLEO DA ALEGRIA') !== false) {
	$html = file_get_contents($url . "rel/rendimento_nucleo_html2021.php?id=$id");
	echo $html;
}

if (strpos($nome_escola, 'CRECHE MUNICIPAL PROFª MARIA MADALENA') !== false and $serie < 3) {
	$html = file_get_contents($url . "rel/rendimento_creche_html2021.php?id=$id");
	echo $html;
}
